...
from .utils import logger, sleeping
from .config import GALXE_DATA
import settings
...
class Browser:
            ...
            try:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        raise ValueError(f'Galxe | Not found addressInfo.id: {error}\n{response.text}')
                else:
    ...
    def reload_task(self, task_name: str):
        ...
        cred_id = GALXE_DATA[task_name]['cred_id']
        ...
        elif cred_id == '367883082841890816':
            payload['variables']['input']['syncOptions']['quiz'] = {"answers": ["2", "3", "3", "3", "0"]}
        ...
        if 'failed to verify recaptcha token' in r.text:
            ...
            sleep(10)
            return self.reload_task(task_name=task_name)

        elif cred_id == '287387120990593025':
            return r.json()['data']['syncCredentialValue']['value']['gitcoinPassport']['score'] >= 20

        elif cred_id == '367883082841890816':
            return r.json()["data"]["syncCredentialValue"]['value']['quiz']['allow']

        elif r.json()['data']['syncCredentialValue']['value']['allow'] == True:
            logger.info(f'[+] Galxe | Task "{task_name}" verified')
            return True

        else:
            logger.warning(f'[-] Galxe | Task "{task_name}" not verified: {r.json()["data"]["syncCredentialValue"]["message"]}')
            return r.json()["data"]["syncCredentialValue"]["message"]


    def claim_campaign(self, task_name: str, retry=0):

        campaign_id = GALXE_DATA[task_name]['campaign_id']
        ...
        if 'failed to verify recaptcha token' in response.text:
            logger.warning(f'[-] Galxe | Outdated `w` token! Dont stop this soft, just change it in `w.txt`')
            sleep(10)
            return self.claim_campaign(task_name=task_name)

        elif response.status_code == 200:
            if response.json()['data']['prepareParticipate']['disallowReason'] != "":
                if response.json()["data"]["prepareParticipate"]["disallowReason"] == 'empty rewards':
                    logger.warning(f'[+] Galxe | Claim "{task_name}" warn: empty rewards')
                    return f'Claim "{task_name}" warn: empty rewards'
                raise Exception(f'[-] Galxe | Claim points error in "{task_name}": {response.json()["data"]["prepareParticipate"]["disallowReason"]}')

            elif response.json()['data']['prepareParticipate']['loyaltyPointsTxResp'] == None:
                logger.info(f'[+] Galxe | Already claimed points in "{task_name}"')
            ...
            logger.error(f'[-] Galxe | Claim points error in "{task_name}": {response.status_code} {response.text}')
            retry += 1
            if retry >= self.max_retries: raise Exception(f'[-] Galxe | Claim points out of attempts in "{task_name}": {response.text}')
    ...
    def open_task(self, task_name: str):
        ...
        if 'failed to verify recaptcha token' in r.text:
            logger.warning(f'[-] Galxe | Outdated `w` token! Dont stop this soft, just change it in `w.txt`')
            sleep(10)
            return self.open_task(task_name=task_name)
        elif task_name != 'quiz':
            if r.json() != {"data": {"typedCredentialItems": {"id": cred_id, "__typename": "Cred"}}}:
                logger.warning(f'[-] Galxe | Cant open quest on Galxe "{task_name}": {r.json()}')
                sleep(10)
                return self.open_task(task_name=task_name)
        else:
            if not r.json()['data']['syncCredentialValue']['value']['quiz']['allow']:
                logger.warning(f'[-] Galxe | Cant open quest on Galxe "{task_name}": {r.json()}')
                sleep(10)
                return self.open_task(task_name=task_name)
    ...
    def complete_quest(self, task_name: str):
        try:
            if task_name in ["faucet", "proof_of_liq", "docs"]:
                self.open_task(task_name=task_name)
                sleep(10)

            reload_status = self.reload_task(task_name=task_name)
            if reload_status == True:
                if GALXE_DATA[task_name]['campaign_id'] != 'GC433ttn6N':
                    return self.claim_campaign(task_name=task_name)
                else:
                    return True

            elif task_name in ["faucet", "proof_of_liq", "docs"] and 'Please click the Go button and visit the link first.' in reload_status:
                return self.complete_quest(task_name=task_name)
    ...
    def get_points_amount(self):
        ...
        if 'failed to verify recaptcha token' in r.text:
            logger.warning(f'[-] Galxe | Outdated `w` token! Dont stop this soft, just change it in `w.txt`')
            sleep(10)
            return self.get_points_amount()
        else:
            for project in r.json()['data']['addressInfo']['loyaltyPointsRanks']['edges']:
                if project['node']['space']['name'].lower() == 'berachain':
                    logger.info(f'[+] Galxe | Total points: {project["node"]["points"]} (Updating with delay)')
                    return project["node"]["points"]
